<aside class="flex flex-col h-screen bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-all duration-300 z-40 fixed left-0 top-0"
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-2 space-y-1">
        @php
            $menuItems = [
                ['label' => 'Dashboard', 'icon' => 'grid-2', 'route' => route('teacherpanel.index')],
                ['label' => 'Planejamento', 'icon' => 'book-sparkles', 'route' => route('planning.lesson-plans.index')],
                ['label' => 'Diário', 'icon' => 'calendar-check', 'route' => route('classrecord.index')],
                ['label' => 'Biblioteca', 'icon' => 'books', 'route' => route('library.index')],
                ['label' => 'Alunos', 'icon' => 'users', 'route' => route('classrecord.students.index')],
                ['label' => 'Suporte', 'icon' => 'headset', 'route' => route('support.index')],
            ];
        @endphp
            ...
            <a href="{{ $item['route'] }}"
               ...
               x-tooltip="sidebarCollapsed ? '{{ $item['label'] }}' : ''">
                ...
                <div class="w-6 flex justify-center">
                    <x-icon :name="$item['icon']" style="solid" class="group-hover:scale-110 transition-transform" />
                </div>
    ...
    <div class="p-4 border-t border-slate-200 dark:border-slate-800">
        ...
        <div class="mt-4 flex justify-between" x-show="!sidebarCollapsed">
             <button class="text-slate-400 hover:text-indigo-500" x-tooltip="'Configurações'">
                <x-icon name="gear" style="solid" />
             </button>
             <button class="text-slate-400 hover:text-red-500" x-tooltip="'Sair'">
                <x-icon name="arrow-right-from-bracket" style="solid" />
             </button>
        </div>
    </div>
</aside>
