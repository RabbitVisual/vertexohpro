<x-teacherpanel::layouts.master title="Editar Turma">
    <div class="p-6 max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Editar Turma</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">Atualize os detalhes da turma "{{ $class->name }}".</p>
            </div>
        </div>

        <!-- Form Card -->
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            <form action="{{ route('school-classes.update', $class->id) }}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ $class->name }}" required class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-indigo-500 transition-all text-slate-900 dark:text-white placeholder-slate-400" placeholder="Ex: 6º Ano A">
                    </div>

                    <!-- Subject -->
                    <div class="space-y-2">
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300">Disciplina / Matéria</label>
                        <input type="text" name="subject" value="{{ $class->subject }}" required class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-indigo-500 transition-all text-slate-900 dark:text-white placeholder-slate-400" placeholder="Ex: Geografia">
                    </div>

                    <!-- Year -->
                    <div class="space-y-2">
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300">Ano Letivo</label>
                        <input type="text" name="year" value="{{ $class->year }}" required class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-indigo-500 transition-all text-slate-900 dark:text-white placeholder-slate-400">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="grade" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-indigo-500 transition-all text-slate-900 dark:text-white">
                            @for($i = 1; $i <= 9; $i++)
                                <option value="{{ $i }}" {{ $class->grade == $i ? 'selected' : '' }}>{{ $i }}º Ano</option>
                            @endfor
                            <option value="10" {{ $class->grade == 10 ? 'selected' : '' }}>Ensino Médio</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="p-4 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 flex items-start gap-4">
                    <div class="flex items-center h-5">
                        <input type="checkbox" name="is_multigrade" value="1" {{ $class->is_multigrade ? 'checked' : '' }} class="w-5 h-5 text-indigo-600 rounded-lg border-slate-300 focus:ring-indigo-600 transition-all">
                    </div>
                    <div class="text-sm">
